...
ItemType ItemDB::parseType(const QString& s)
{
    ...
    if (t == "food")   return ItemType::Food;
    if (t == "weapon") return ItemType::Weapon;
    if (t == "shield") return ItemType::Shield;
    if (t == "monster")return ItemType::Monster;
...
ItemDef ItemDB::loadOneItemDir(const QString& id, const QString& dirPath, const QSize& targetSize)
{
        ...
        if (f.exists() && f.open(QIODevice::ReadOnly)) {
            ...
            if (doc.isObject()) {
                ...
                if (o.contains("tags") && o["tags"].isArray()) {
                    ...
                    for (auto v : arr)
                        if (v.isString()) def.tags.push_back(v.toString());
                }
    ...
    for (const auto& fi : files) {
        ...
        if (!norm.isNull()) def.frames.push_back(norm);
    }
...
{
    items.clear();
    if (assetsRoot.isEmpty()) return false;

    QDir itemsDir(QDir(assetsRoot).filePath("items"));
    if (!itemsDir.exists()) return false;
    ...
    for (const auto& d : dirs) {
        ...
        if (def.frames.isEmpty()) continue; // 没帧就忽略
        items.insert(id, def);
    }
